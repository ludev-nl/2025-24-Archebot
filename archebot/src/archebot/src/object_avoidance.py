...
# def test_object_avoidance():
#     avoider = Avoider()
#     avoider.initialized = True
#     avoider.corrected = FILLIN
#     avoider.highest_idx = FILLIN
#     avoider.lowest_idx = FILLIN
#
...
class Avoider:
    def __init__(self):
        ...
        self.view_sections = [
            (i * self.section_width, (i + 1) * self.section_width)
    ...
    def detect_object(self, data):
        ...
        if np.any(
            self.corrected[:, self.view[1][0] : self.view[1][1]] - arr_view > 0
        ):  # detect only exactly in front
            # if left detects oject
            if np.any(
                self.corrected[
                    :,
                    self.view[1][0] + self.view_sections[0][0] : self.view[1][0]
                    + self.view_sections[0][1],
                ]
                - arr_view[:, self.view_sections[0][0] : self.view_sections[0][1]]
                > 0
            ):
            ...
            # right detects object
                ...
                self.corrected[
                    :,
                    self.view[1][0] + self.view_sections[1][0] : self.view[1][0]
                    + self.view_sections[1][1],
                ]
                - arr_view[:, self.view_sections[1][0] : self.view_sections[1][1]]
                > 0
            ):
